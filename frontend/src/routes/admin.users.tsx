  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10">
      ...
      <section className="mt-6 grid gap-3 rounded-2xl border border-border bg-card p-4 shadow-sm md:grid-cols-[1fr_180px_200px]">
        ...
        <select value={role} onChange={(event) => setRole(event.target.value as RoleFilter)} className="min-h-10 rounded-md border border-input bg-field px-3 dark:bg-background text-sm"><option value="">{t("admin.users.role")}</option><option value="USER">USER</option><option value="ADMIN">ADMIN</option></select>
        <select value={status} onChange={(event) => setStatus(event.target.value as StatusFilter)} className="min-h-10 rounded-md border border-input bg-field px-3 dark:bg-background text-sm"><option value="">{t("admin.users.status")}</option><option value="ACTIVE">{statusLabel("ACTIVE", lang)}</option><option value="PENDING_DELETION">{statusLabel("PENDING_DELETION", lang)}</option><option value="BANNED">{statusLabel("BANNED", lang)}</option></select>
      </section>
      <div className="mt-7">
