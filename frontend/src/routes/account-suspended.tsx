    ...
    <AuthShell title={t("auth.reactivate.title")} subtitle={t("auth.reactivate.subtitle")}>
      ...
      {success ? (
        ...
        <form onSubmit={submit} className="space-y-4" noValidate>
          ...
          <Button type="button" variant="outline" className="w-full" onClick={() => void router.navigate({ to: "/reset-password" })}><KeyRound className="mr-2 h-4 w-4" />{t("auth.login.forgot")}</Button>
